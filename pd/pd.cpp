...
int _tmain(int argc, _TCHAR* argv[])
{
		...
		(int) flagDB_gen + (int) flagDB_genQuick + (int) flagDB_add + (int) flagDB_clean + (int) flagDumpCloses > 1 )
	{
		...
		fprintf(stderr,"Error. Only one PD or hash database command should be issued per execution.\r\n");
		exit(0);
	}
	...
	{
		printf("WARNING: To properly access all processes on a 64 bit Windows version, the 64 bit version of this tool should be used. Currently Pd is running as a 32bit process under a 64bit operating system.\r\n\r\n");
	}
	...
	else if (flagDumpCloses)
	{
		...
		if (SetConsoleCtrlHandler((PHANDLER_ROUTINE)ConsoleHandler, TRUE) == FALSE)
		{
			...
			printf("WARNING: Unable to install keyboard handler. This means that pd will not be able to close or cleanup properly.\r\n");
		}
